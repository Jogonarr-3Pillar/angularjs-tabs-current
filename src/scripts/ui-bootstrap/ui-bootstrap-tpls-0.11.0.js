            ...
            this.createParser = function (format) {
                ...
                angular.forEach(formatCodeToRegex, function (data, code) {
                    ...
                    if (index > -1) {
                        ...
                        for (
                            var i = index + 1, n = index + code.length;
                            i < n;
                            i++
                        ) {
                            regex[i] = '';
        ...
        function (
            ...
            $scope.select = function (date) {
                if ($scope.datepickerMode === self.minMode) {
                    ...
                    $scope.datepickerMode =
                        self.modes[
                            self.modes.indexOf($scope.datepickerMode) - 1
                        ];
                }
            };
            ...
            $scope.move = function (direction) {
                var year =
                        self.activeDate.getFullYear() +
                        direction * (self.step.years || 0),
                    month =
                        self.activeDate.getMonth() +
            ...
            $scope.toggleMode = function (direction) {
                ...
                if (
                    ($scope.datepickerMode === self.maxMode &&
                        direction === 1) ||
                ...
                $scope.datepickerMode =
                    self.modes[
                        self.modes.indexOf($scope.datepickerMode) + direction
                    ];
            };
                ...
                link: function (scope, element, attrs, ctrl) {
                    ...
                    ctrl._refreshView = function () {
                        var year = ctrl.activeDate.getFullYear(),
                            ...
                            difference =
                                ctrl.startingDay - firstDayOfMonth.getDay(),
                        ...
                        if (scope.showWeeks) {
                            scope.weekNumbers = [];
                            var weekNumber = getISO8601WeekNumber(
                                    scope.rows[0][0].date
                                ),
                                numWeeks = scope.rows.length;
                            while (
                                scope.weekNumbers.push(weekNumber++) < numWeeks
                            ) {}
                        }
                    };
                    ...
                    function getISO8601WeekNumber(date) {
                        ...
                        return (
                            Math.floor(
                                Math.round((time - checkDate) / 86400000) / 7
                            ) + 1
                        );
                    }
                ...
                link: function (scope, element, attrs, ctrl) {
                    ...
                    ctrl.handleKeyDown = function (key, evt) {
                        ...
                        if (key === 'left') {
                            ...
                            date +=
                                (key === 'pageup' ? -1 : 1) * ctrl.step.years;
                        } else if (key === 'home') {
                ...
                link: function (scope, element, attrs, ngModel) {
                    ...
                    function parseDate(viewValue) {
                        if (!viewValue) {
                            ...
                            var date =
                                dateParser.parse(viewValue, dateFormat) ||
                                new Date(viewValue);
                            if (isNaN(date)) {
    ...
    .controller('DropdownController', [
        ...
        function (
            ...
            this.toggle = function (open) {
                return (scope.isOpen = arguments.length
                    ? !!open
                    : !scope.isOpen);
            };
        ...
        function (
            ...
            $modalStack.open = function (modalInstance, modal) {
                ...
                if (currBackdropIndex >= 0 && !backdropDomEl) {
                    ...
                    backdropDomEl = $compile('<div modal-backdrop></div>')(
                        backdropScope
                    );
        ...
        function ($parse, paginationConfig) {
            return {
                ...
                link: function (scope, element, attrs, ctrls) {
                    ...
                    scope.directionLinks = angular.isDefined(
                        attrs.directionLinks
                    )
                    ...
                    function getPages(currentPage, totalPages) {
                        ...
                        if (isMaxSized) {
                            if (rotate) {
                                ...
                                startPage = Math.max(
                                    currentPage - Math.floor(maxSize / 2),
                                    1
                                );
                                ...
                                startPage =
                                    (Math.ceil(currentPage / maxSize) - 1) *
                                        maxSize +
                                    1;
            ...
            function (
                ...
                return function $tooltip(type, prefix, defaultTriggerShow) {
                    ...
                    function getTriggers(trigger) {
                        var show =
                            trigger || options.trigger || defaultTriggerShow;
                    ...
                    return {
                        ...
                        compile: function (tElem, tAttrs) {
                            ...
                            return function link(scope, element, attrs) {
                                ...
                                var popupTimeout;
                                var appendToBody = angular.isDefined(
                                    options.appendToBody
                                )
                                    ? options.appendToBody
                                    : false;
                                var triggers = getTriggers(undefined);
                                var hasEnableExp = angular.isDefined(
                                    attrs[prefix + 'Enable']
                                );
                                ...
                                function showTooltipBind() {
                                    if (
                                        hasEnableExp &&
                                        !scope.$eval(attrs[prefix + 'Enable'])
                                    ) {
                                        return;
                                    }
                                ...
                                attrs.$observe(
                                    prefix + 'Placement',
                                    function (val) {
                                        scope.tt_placement = angular.isDefined(
                                            val
                                        )
                                            ? val
                                            : options.placement;
                                ...
                                var animation = scope.$eval(
                                    attrs[prefix + 'Animation']
                                );
                                scope.tt_animation = angular.isDefined(
                                    animation
                                )
                                    ? !!animation
                                    : options.animation;
    ...
    .controller('TimepickerController', [
        ...
        function ($scope, $attrs, $parse, $log, $locale, timepickerConfig) {
            ...
            function updateTemplate(keyboardChange) {
                ...
                $scope.minutes =
                    keyboardChange === 'm' ? minutes : pad(minutes);
                $scope.meridian =
        ...
        function (
            ...
            return {
                ...
                link: function (originalScope, element, attrs, modelCtrl) {
                    ...
                    var minSearch =
                        originalScope.$eval(attrs.typeaheadMinLength) || 1;
                    ...
                    var getMatchesAsync = function (inputValue) {
                        ...
                        $q.when(
                            parserResult.source(originalScope, locals)
                        ).then(
                            function (matches) {
    ...
    function ($templateCache) {
        $templateCache.put(
            ...
            '<li role="presentation" \n' +
                ...
                '  <a href="javascript:void(0);" ng-click="select()" role="tab" aria-selected="{{!!active}}" aria-current="{{!!active ? \'page\' : false }}" tab-heading-transclude>{{heading}}</a>\n' +
                '</li>\n' +
                ''
        );
    },
